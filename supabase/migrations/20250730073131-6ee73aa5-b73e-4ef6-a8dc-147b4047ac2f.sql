-- Corrigir a função de sorteio para transferir o token para o comprador
...
BEGIN
  ...
  BEGIN
    ...
    -- Adicionar pontos ao perdedor (valor base do token como compensação)
    ...
    -- Adicionar pontos ao comprador (valor base do token)
    UPDATE public.user_scores 
    SET score = score + p_token_points
    WHERE user_id = p_buyer_id;
